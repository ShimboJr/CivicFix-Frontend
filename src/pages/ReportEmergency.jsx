 ...
 *   1. EmergencyCallBanner — unconditional, always first, above every form field
 *   2. Framing notice      — "This is not a replacement for calling emergency services"
 *   3. Report form         — type, description, location, media capture
 *
...
// Max video duration in seconds — must match server-side MAX_VIDEO_DURATION_SECONDS - 1
const VIDEO_MAX_SECONDS = 15;
// Max total files (images + video) — must match server-side MAX_FILES_PER_REPORT
const MAX_MEDIA_FILES = 5;
// Video capture constraints — 720p max, ~2 Mbps
...
export default function ReportEmergency() {
  ...
  // ── Media state ──────────────────────────────────────────────────────────
  // `mediaFiles` is an array of { file: File, preview: string, kind: 'photo'|'video' }
  ...
  const [cameraStream,    setCameraStream]    = useState(null); // for live preview

  const mediaRecorderRef  = useRef(null);
  const chunksRef         = useRef([]);
  const countdownTimerRef = useRef(null);
  const stopTimerRef      = useRef(null);
  const livePreviewRef    = useRef(null); // <video> element for live viewfinder

  // Photo file input
  const photoRef   = useRef();
  // Gallery (existing video) file input
  ...
  const removeMediaFile = (idx) => {
    setMediaFiles((prev) => {
      URL.revokeObjectURL(prev[idx].preview); // release object URL
  ...
  // NOTE: gallery videos bypass capture-time resolution/duration constraints.
  // The server-side duration (16 s gate) and size (25 MB) checks in
  // emergencyUploadMiddleware.js are the backstop for this path.
  // Client-side compression via ffmpeg.wasm would reduce upload size further
  // but is deferred due to the ~20 MB bundle-size cost.
  ...
  const startRecording = async () => {
    ...
    // Show live viewfinder
    if (livePreviewRef.current) {
      livePreviewRef.current.srcObject = stream;
      livePreviewRef.current.play().catch(() => {});
    }

    // Pick a supported MIME type — ordered by preference
    ...
    recorder.onstop = () => {
      ...
      // Tear down the live preview stream
      stream.getTracks().forEach((t) => t.stop());
    ...
    // Enforce server-side: a modified client that skips the frontend could
    // bypass this timer, which is why the server-side check in
    // emergencyUploadMiddleware.js (checkVideoDurations) exists as a backstop.
  ...
  const handleSubmit = async (e) => {
    ...
    // Append all media files under the 'media' field name (matching the
    // Multer field name configured in emergencyUploadMiddleware.js)
    ...
    return (
      <DashboardLayout title="Report Submitted">
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          {/* Repeat the call banner at the top of the success screen too */}
          <EmergencyCallBanner phoneNumber={null} />
          ...
          <div className="cf-card" style={{ textAlign: 'center', padding: '2.5rem 2rem' }}>
            ...
            {/* Repeat safety message — required by the spec */}
            <div style={{
              background: '#fff5f5',
  ...
  return (
    <DashboardLayout title="Report Emergency">
      {/*
        DashboardLayout's `title` prop renders an <h1> above {children}.
        We want the call banner to appear ABOVE the form but BELOW the h1
        (the h1 is inside DashboardLayout's main container, before children).
        Because DashboardLayout renders title then children, we put the banner
        as the first child here — it appears immediately below the h1, before
        any form field, and without any scrolling needed on a normal viewport.
      */}
      <div style={{ maxWidth: 720 }}>
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="cf-card" style={{ marginBottom: '1.25rem' }}>
                ...
                {mediaFiles.map(({ preview, kind }, idx) => (
                  <div key={idx} style={{ position: 'relative', width: 90, height: 90, borderRadius: 'var(--cf-radius-md)', overflow: 'hidden', border: '1px solid var(--cf-border)' }}>
                    {kind === 'photo' ? (
                      ...
                      /* Video thumbnail — show a video element in thumbnail mode */
                      <video
                        src={preview}
                        style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                        muted
                    ...
                    <button
                      ...
                      style={{
                        ...
                        justifyContent: 'center', fontSize: '0.7rem', padding: 0,
                      }}
                    >
                      <i className="bi bi-x" />
                    </button>
            ...
            {/* Live viewfinder during recording */}
            {recording && (
              <div style={{
                position: 'relative', marginBottom: '1rem',
                borderRadius: 'var(--cf-radius-md)', overflow: 'hidden',
                ...
                <div style={{
                  ...
                  background: countdown <= 5 ? '#dc2626' : 'rgba(0,0,0,0.65)',
                  color: '#fff', borderRadius: 'var(--cf-radius-md)',
                ...
                <div style={{
                  ...
                  <span style={{
                    ...
                    background: '#ef4444',
                    animation: 'ecb-ring 1s ease-out infinite',
                  }} />
                  REC
                </div>
              </div>
            ...
            {/* Three capture action buttons — shown when not at the file limit */}
            {canAddMore && !recording && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.6rem', marginBottom: '0.75rem' }}>

                {/* A. Take Photo — triggers device camera directly */}
                <button
                  type="button"
                  id="er-take-photo-btn"
                ...
                {/* B. Record Video — MediaRecorder with enforced constraints */}
                <button
                  type="button"
                  id="er-record-video-btn"
                ...
                {/* C. Choose from Gallery — plain file input, bypasses capture constraints */}
                {/* NOTE: gallery videos skip the capture-time 720p/2Mbps constraints above.
                    The server-side emergencyUploadMiddleware.js checks are the backstop:
                      - Size: 25 MB limit enforced before Cloudinary upload
                      - Duration: 16 s gate via Cloudinary's `duration` field post-upload
                    Client-side ffmpeg.wasm compression is a valuable follow-up for this
                    path but deferred due to ~20 MB bundle-size cost. */}
                <button
                  type="button"
                  id="er-gallery-btn"
            ...
            {/* Stop recording button — shown only while recording */}
            {recording && (
              <button
                type="button"
                id="er-stop-recording-btn"
            ...
            {mediaFiles.length > 0 && (
              <p style={{ fontSize: '0.78rem', color: 'var(--cf-text-muted)', margin: '0.35rem 0 0' }}>
                {mediaFiles.length}/{MAX_MEDIA_FILES} files added
          ...
          {/* ── Hidden file inputs ────────────────────────────────────── */}
          {/* Photo: capture="environment" opens rear camera on mobile */}
          <input
            ref={photoRef}
            type="file"
          ...
          />
          {/* Gallery: accept video files — no capture attribute so it opens gallery */}
          <input
            ref={galleryRef}
            type="file"
